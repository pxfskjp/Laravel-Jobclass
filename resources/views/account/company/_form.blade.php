...
<div id="companyFields">
	...
	<!-- Business License -->
	<?php $companyLicenseError = (isset($errors) and $errors->has('company.license')) ? ' is-invalid' : ''; ?>
	<div class="form-group row">
		<label class="{{ $classLeftCol }} control-label{{ $companyLicenseError }}" for="company.license"> {{ t('License') }} </label>
		<div class="{{ $classRightCol }}">
			<div {!! (config('lang.direction')=='rtl') ? 'dir="rtl"' : '' !!} class="file-loading mb10">
				<input id="license" name="company[license]" type="file" class="file{{ $companyLicenseError }}">
			</div>
			<small id="" class="form-text text-muted">
				{{ t('File types: :file_types', ['file_types' => showValidFileTypes('image')]) }}
			</small>
...
@section('after_scripts')
	...
	<script>
		...
		/* Initialize with defaults (license) */
		$('#license').fileinput(
		{
			theme: "fa",
			language: '{{ config('app.locale') }}',
			...
			minFileSize: {{ (int)config('settings.upload.min_image_size', 0) }}, {{-- in KB --}}
			maxFileSize: {{ (int)config('settings.upload.max_image_size', 1000) }}, {{-- in KB --}}
			@if (isset($company) and !empty($company->license) and isset($disk) and $disk->exists($company->license))
				/* Retrieve Existing license */
				initialPreview: [
					'<img src="{{ imgUrl($company->license, 'medium') }}" class="file-preview-image">'
				],
			@endif
			/* Remove Drag-Drop Icon (in footer) */
			fileActionSettings: {dragIcon: '', dragTitle: ''},
