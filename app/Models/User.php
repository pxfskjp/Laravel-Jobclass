...
class User extends BaseUser
{
    ...
    */
    ...
    public static function getProfile($skills)
	{
        $profile = collect(User::where(['user_type_id'=>User::JOBSEEKER])->get());
        $res =[];
        
        foreach($profile as $prof){
            // get user 's property
            $usr =  collect($prof->attributes);
            $marks = User::getMarks(jsonToArray($usr['skills']),$skills);
            // dd(collect($marks)->count());
            $usr->put('marks',$marks);
            $antimarks = 10;
            if(collect($marks)->count()){
                $antimarks = 10-(collect($marks)->sum('value')/collect($marks)->count());
            }
            $usr->put('antimark',$antimarks);
            array_push($res,$usr);
        }
        // dd($res);
		view()->share('profile', collect($res)->sortby('antimark'));
    }
    public static function getMarks($user,$post)
    {
        $marks = [];
        $index = 0;
        $user = collect($user);
        foreach($post as $item){
            $u = collect($user->where('id',$item['id'])->first());
            array_push($marks,['name'=>$item['name'],'value'=>($u->count())?$u['score']:0 ]);
        }
        return $marks;
    }
