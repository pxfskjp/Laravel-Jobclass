                                            ...
                                            @else
                                                ...
                                                */
												...
												<div id="skillBox" class="form-group row required">
													<label class="col-md-3 col-form-label{{ $skillsError }}" for="skills">Skills <sup>*</sup></label>
													<div class="col-md-9">
														<select id="skills" size = 2 name="skills[]" class="sselecter{{ $skillsError }} form-control" multiple>
															@if($skills)
																@foreach($skills as $skill)
																<option value="{{$skill->id}}" selected >{{$skill->name}} : [{{$skill->score}}]</option>
																@endforeach
															@endif
														</select>
													</div>
...
@section('after_scripts')
	<script>
		$("#skills").on('select2:unselect', function (event) {
				...
				console.log("selectedId",$selectedOptionClass);
