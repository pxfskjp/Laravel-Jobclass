...
trait EditTrait
{
	...
	public function getUpdateForm($postIdOrToken)
	{
		...
		//Get the Post's Require Skills
		$requireSkills = DB::select("select require_skills from posts where id='${postIdOrToken}'")[0];
		$skills = json_decode($requireSkills->require_skills);
		$data['require_skills'] = $skills;

		// Get Post's mandatestate
		$data['mandatestate'] = $post['mandatestate_id'];
	...
	public function postUpdateForm($postIdOrToken, PostRequest $request)
	{
		...
		//// for error message
		$requireSkills = $request->input('require_skills');
		if(!$requireSkills) {
