...
class EditController extends AccountBaseController
{
	...
	public function index()
	{
		...
		$skillsListTemp = auth()->user()->skills;
		$skillsListTemp = json_decode($skillsListTemp);
		if($skillsListTemp)
		foreach($skillsListTemp as $listTemp){
			$resultName = DB::select('select name from skills where id = (?)',[$listTemp->id]);
			$listTemp->name = $resultName[0]->name;
		}
		$data['skills'] = $skillsListTemp;
	...
	public function updateDetails(UserRequest $request)
	{
		...
		if($request->input('skills')){
			...
			$temp = $request['email'];
			DB::update('update users set skills = (?) where email = (?)',[$skill_list,$temp]);
		}
